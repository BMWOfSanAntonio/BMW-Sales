<template>
  <v-container>
    <div class="display-3">Completed We Owes</div>
    <v-simple-table>
      <thead>
        <tr>
      ...
      <tbody>
        <tr v-for="(weowe, i) in weowes" :key="i">
          ...
          <td v-else-if="weowe.parts_status == 'Complete' && weowe.parts_on_order == 'Yes'">
            <v-chip color="success">{{ weowe.parts_status }} by {{ weowe.parts_associate }}</v-chip>
            <v-chip color="error">Parts on Order</v-chip>
          </td>
...
export default {
  ...
  firestore() {
    return {
      weowes: db.collection("weowe").where("parts_state", "==", false)
    };
  }
};
</script>
